...
| **R-k** | **`ground_cost.py` benches without resetting the estimator** | **Found 2026-09-18 by `tests/test_ground_reset_convention.py` on its first run.** `reports/harnesses/ground_cost.py` warms up with one `segment_ground` call and then runs `bench(..., reps=3)` over the same scans **without ever resetting**, so reps 2 and 3 begin from the state rep 1 left behind — which a real run never does. It is a committed provenance harness: it produced R-a's isolated ground cost (**21.01 ms p50**, `reports/latency-gap-investigation.md`), and that figure is quoted again in **D8**'s 4.4x variance note (21.01 -> 93.44 ms). **Deliberately NOT fixed here:** editing the harness would change the provenance of a published number, and this laptop was paging, so re-measuring now would produce an untrusted figure either way. It is listed as an explicit, non-growing exception in that test (`KNOWN_UNRESET`), with a companion test that fails if it is ever fixed and left on the list. **[2026-09-18] Deterministic half MEASURED and the published figure SURVIVES.** `reports/harnesses/ground_estimator_carryover.py` reproduces `ground_cost.py`'s exact state sequence (warm-up + 3 reps x 60 scans) and compares it against a fresh estimator on the same scans in the same order. **Agreement: 96.5% fresh vs 96.5% carried-over (+0.07 points)** — R-a's published 96.5% is not invalidated and needs no restatement. **But the masks do differ:** 10 of 20 frames, **5,878 of 2,471,164 points reclassified (0.2379%)**, first differing frame 0. So the carry-over is real and does change which points are called ground, at a quarter of a percent — enough to perturb per-frame map content, not enough to move the headline. Confirmed by a second run in a fresh process, identical to the digit. **Still open: the TIMING half.** This laptop was paging (commit 22.59 GB > 15.73 GB physical), so no latency figure was taken and the 21.01 ms p50 is neither confirmed nor challenged here. **Needs:** one pass on a state-trusted machine — reset per rep, re-time, and decide whether 21.01 ms restates. | JP (it is his R-a figure) | `tests/test_ground_reset_convention.py`, `reports/harnesses/ground_cost.py` |
