...
|---|---|---|---|---|
| **D1** | **The Patchwork++ singleton lifetime.** Does `segment_ground` take an estimator? Does the module expose a reset? Who owns the lifetime? | JP (design) | Three real options, none obviously right. **Highest-impact item on this list** — see §2. The determinism test `test_real_sequence_replay_is_identical` fails because of it. | `reports/ring1-reproduction-investigation.md` §7 |
