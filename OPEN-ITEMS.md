...
|---|---|---|---|---|
| **D1** | **The Patchwork++ singleton lifetime.** Does `segment_ground` take an estimator? Does the module expose a reset? Who owns the lifetime? | JP (design) | **[2026-09-18] The point-fix landed UPSTREAM, not on this branch.** JP reports that Aakash added `reset_estimator()` calls at the script/harness entry points in `03abd7b` (14–15 Sep). **Not verifiable in this clone:** `03abd7b` is absent, and the symbol `reset_estimator` appears in no commit on any local branch; the newest ref here is `vrgrid26/main` @ `0c769c9` (13 Sep). Our own determinism test still fails locally (re-run 2026-09-18), as expected for a clone without that fix. **Nothing on `jp/p99-alloc-fixes` fixed D1** — every harness here resets the module global itself (`ground._estimator = None`) without editing `ground.py`. **The open question is unchanged and is NOT the point-fix:** who owns the estimator's lifetime, whether `segment_ground` should take one, and whether the API should expose an explicit reset. A reset at entry points is a call-site convention, not a lifetime design, and it leaves the same trap for any new caller that forgets it. See `reports/pre-merge-collision-check.md`. Three real options, none obviously right. **Highest-impact item on this list** — see §2. The determinism test `test_real_sequence_replay_is_identical` fails because of it. | `reports/ring1-reproduction-investigation.md` §7 |
| **D2** | **R3 — ring boundary under anisotropy.** Nearest-corner test + boundary snapping. | Aakash | Touches `src/grid/lattice.py` (his) and has a cross-lane consequence in `metrics._ring_cells`. Relevant to SIH26053's "without alignment errors or data loss". | `pending-review/r3-ring-boundary-under-anisotropy.md` |
...
- `test_real_sequence_replay_is_identical` fails, correctly. Two replays in one process differ by
  ...
  2026-09-14).
- **[2026-09-18]** It still fails **in this clone**, which does not contain the upstream point-fix JP reports (`03abd7b`, unverifiable here). Whether upstream's test now passes is unknown from here.
