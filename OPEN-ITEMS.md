...
| **R-h** — `cleanup` / `ground` | **`cleanup` DONE and proposed; `ground` deferred as a different mechanism.** `cleanup`'s 9.61 MB is **not** in `visibility_cleanup` (scratch-based, allocation-free as documented) — it is `np.isin` inside an argument to an otherwise careful `np.copyto`. A boolean LUT is **21x less allocation, 3.9x faster, bit-identical**. `ground` stays open: largest spread, modest allocation, inside the C++ extension. | `pending-review/cleanup-isin-guard.md` |
