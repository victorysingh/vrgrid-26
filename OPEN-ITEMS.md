...
| **D10** | **ROS 2 adapter scope** — window extent, bitfield as one float layer or six boolean ones, where live labels come from, publish rate, and whether `export_gridmap()` needs arguments. | The last of those changes a **frozen signature**, so it needs the three-way *before* implementation rather than halfway through. **And the blocker is throughput, not the wire format:** `query()` is 54.7 µs, so a 20 m window at 5 cm is **8.75 s per frame (0.11 Hz)**. A vectorised bulk reader is a prerequisite and must be pinned bit-identical to `query_region`. | `pending-review/ros2-adapter-design.md` §4, §7 |
...
| **R-g** | **Extend the no-allocation invariant to the perception half.** `test_the_two_grid_allocations_stay_fixed` covers `MapEngine.step` only; `transform_points` allocates 10.86 MB/frame and nothing notices. | Independent of D9 — worth doing whichever way D9 goes, because the same regression can reappear anywhere in `iter_pipeline`. |
| **R-h** | **The next two p99 contributors: `ground` (42% of the warm-run excess) and `cleanup` (32%).** `ground` is inside the C++ extension so harder to attribute; `cleanup` is our own numpy and is the better next target. | R-b localised `transform`; these are what remain. |
| **R-i** | **Instrument front-end allocation.** `--alloc` covers the mapping back end only; the perception stages are unmeasured by the tool, which is how a back-end figure became a whole-frame claim. | JP asked for this as "R-h", but **that label was already in use** (the ground/cleanup p99 contributors, added earlier tonight), so it is filed as R-i rather than double-assigned. |
