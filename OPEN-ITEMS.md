...
|---|---|---|---|---|
| **D1 — DECIDED (A)** | **The Patchwork++ singleton lifetime.** Does `segment_ground` take an estimator? Does the module expose a reset? Who owns the lifetime? | JP (design) | **[2026-09-18, VERIFIED after a fetch] The point-fix landed UPSTREAM, not on this branch.** It is **`3c26d47`** (14 Sep, **AakashH2006**), *"determinism: one Patchwork++ estimator was shared by every run in a process"* — **not** `03abd7b`, which is Shrestha's GPU frame-loop commit. Upstream calls `ground.reset_estimator()` inside `iter_pipeline` **before** the frame loop (`src/run/__main__.py:98`, loop at :100), i.e. **once per run, not per frame**, so it does not sit in the timed per-frame path; `harness.py:366`, `feature_report.py:84`, `gen_demo_rrds.py:82` and `gpu_parity.py:76` do the same, and `tests/test_ground.py` pins it. **[2026-09-18, after the merge `ef4524e`] Our determinism test now PASSES on this branch** (`tests/test_determinism.py` 13 passed), because the merge brought `3c26d47` in. The symptom is gone; the design question below is not. **Nothing on `jp/p99-alloc-fixes` fixed D1** — every harness here resets the module global itself (`ground._estimator = None`) without editing `ground.py`. **The open question is unchanged and is NOT the point-fix:** who owns the estimator's lifetime, whether `segment_ground` should take one, and whether the API should expose an explicit reset. A reset at entry points is a call-site convention, not a lifetime design, and it leaves the same trap for any new caller that forgets it. See `reports/pre-merge-collision-check.md`. **[2026-09-18] DECIDED BY JP: option A — keep the module-level estimator and the per-run `reset_estimator()` at entry points. `ground.py` is unchanged and D1 needs no code change.** What A was missing is now supplied: `tests/test_ground_reset_convention.py` (5 tests) enforces the convention instead of trusting it. It walks the AST of `src/`, `scripts/`, `dashboard/` and `reports/harnesses/`, finds every module calling `segment_ground*` directly, and fails naming any that never resets — verified non-vacuous by planting an offending file and watching it fail. It also pins that `iter_pipeline` resets **once per run, not per frame** (per-frame would stay deterministic and silently add a Patchwork++ construction to the timed path), and that the five shipped entry points use the public call. The four plain `ground._estimator = None` pokes were migrated to `reset_estimator()` (a literal no-op); the 9 remaining private assignments all install a *configured* estimator, which is the cost option A knowingly accepts. Background:  — three options (keep the global; optional `estimator=` parameter; a `GroundSegmenter` that owns the lifetime), each with its case for and against, and the evidence that decides between them: **5 calls to the public `reset_estimator()` against 13 assignments to the private `ground._estimator` across 9 files.** Five of those install a *differently configured* estimator for the num-iter work, which the public API cannot express at all — so the real requirement is injection, not just resetting. Recommendation is B-then-C if the goal is to stop the private pokes, A if the goal is not to disturb the GPU lane mid-flight. **`ground.py` untouched; nothing implemented; still JP's call.** **Highest-impact item on this list** — see §2. `test_real_sequence_replay_is_identical` now passes (it failed here before the merge). | `reports/ring1-reproduction-investigation.md` §7 |
