...
def run(label, mutate=None):
    ...
    for s in scans[:3]: est.estimateGround(np.asarray(s, dtype=np.float64))
    masks, ts = [], []
    for s in scans:
        a = np.asarray(s, dtype=np.float64)
        t0 = time.perf_counter(); est.estimateGround(a)
        ts.append((time.perf_counter()-t0)*1e3)
        m = np.zeros(len(a), bool); m[np.asarray(est.getGroundIndices(), dtype=np.int64)] = True
        masks.append(m)
    return label, float(np.median(ts)), masks

def combo_a(p): p.num_iter = 2; p.enable_RNR = False; p.enable_RVPF = False
def combo_b(p): p.num_iter = 1; p.enable_RNR = False; p.enable_RVPF = False

rows = [run("SHIPPED"), run("A: num_iter2+RNRoff+RVPFoff", combo_a),
        run("B: num_iter1+RNRoff+RVPFoff", combo_b)]
base = rows[0][2]; b = rows[0][1]
